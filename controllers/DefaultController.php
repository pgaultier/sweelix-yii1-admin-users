...
class DefaultController extends Controller {
	...
	public function actionIndex() {
		try {
			...
			$usersDataProvider = new \CActiveDataProvider('sweelix\yii1\ext\entities\Author', array('criteria' => $criteria));
			$this->render('list', array('usersDataProvider' => $usersDataProvider));

		} catch(\Exception $e) {
			\Yii::log('Error in '.__METHOD__.'():'.$e->getMessage(), \CLogger::LEVEL_ERROR, 'sweelix.yii1.admin.users.controllers');
			throw $e;
		}
		...
		try {
			...
			if(isset($_POST[Html::modelName($author)])){
				...
				if($author->validate() === true) {
					$attributes = array(
						'authorLastname', 'authorFirstname', 'authorEmail'
					);
	...
	protected function getRolesFromModules() {
		if($this->_rolesFromModules === null) {
			$this->_rolesFromModules = array();
			$modules = array_keys(\Yii::app()->getAuthManager()->getRoles());

			// $values = array_fill(0, count($modules), false);
			$this->_rolesFromModules = array_combine($modules, $modules);
		}
		return $this->_rolesFromModules;
	}
